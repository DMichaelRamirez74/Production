...
namespace Fingerprints.Controllers
{
   ...
   */
        ...
        [JsonMaxLength]
        public JsonResult GetWithdrawnChildren(string centerId, string programId)
        {
            List<WaitingChildren> withdrawn = new List<WaitingChildren>();
            JavaScriptSerializer js = new JavaScriptSerializer();
            string jsonData = string.Empty;
            try
            {
                long centerID = Convert.ToInt64(EncryptDecrypt.Decrypt64(centerId));
                long programID = (programId == "0") ? 0 : Convert.ToInt64(EncryptDecrypt.Decrypt64(programId));
                withdrawn = new ERSEAData().GetWithdrawnChildList(centerID, programID, new Guid(Session["AgencyId"].ToString()));               
            }
            catch (Exception ex)
            {
                clsError.WriteException(ex);
            }
            return Json(withdrawn, JsonRequestBehavior.AllowGet);
        }

        [JsonMaxLength]
        public JsonResult GetDroppedChildren(string centerId, string programId)
        {
            ...
            try
            {
                long centerID = Convert.ToInt64(EncryptDecrypt.Decrypt64(centerId));
                long programID = (programId == "0") ? 0 : Convert.ToInt64(EncryptDecrypt.Decrypt64(programId));
                dropped = new ERSEAData().GetDroppedChildList(centerID, programID, new Guid(Session["AgencyId"].ToString()));
            }
            catch (Exception ex)
            {
                clsError.WriteException(ex);
