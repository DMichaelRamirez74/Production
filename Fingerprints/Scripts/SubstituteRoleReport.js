    ...
    bindAjaxParameters: function (mode, tabEle, index) {
        ...
        switch (mode) {
            case self.parametersMode.filter:
                ...
                self.dataParameters.SearchTerm = '';
                self.dataParameters.Month = self.elements.dropdownMonths.val().join(",");
    ...
    getSubsituteRole: function (ele, index, isCenter) {
        ...
        switch(isCenter)
        {
            case self.getlistMode.center:
                ...
                self.ajaxOptions.url =  self.getSubstituteRoleByCenterUrl ;
                break;
